...
import { laporanSetoranColumns, LaporanSetoran } from './laporanSetoranColumns';
import { KTCard } from '../../../_metronic/helpers/components/KTCard';
import { KTIcon } from '../../../_metronic/helpers';
import { MenuComponent } from '../../../_metronic/assets/ts/components';

const initialData: LaporanSetoran[] = [
    { id: 1, tanggal: '2024-06-01', nama: 'Budi Santoso', jenisSampah: 'Plastik', berat: 5, total: 25000 },
    { id: 2, tanggal: '2024-06-01', nama: 'Siti Aminah', jenisSampah: 'Kertas', berat: 10, total: 30000 },
    { id: 3, tanggal: '2024-06-02', nama: 'Andi Wijaya', jenisSampah: 'Logam', berat: 2, total: 40000 },
];

const LaporanSetoranPage = () => {
  const [data, setData] = useState<LaporanSetoran[]>(initialData);
  ...
  const handleExport = () => {
    const headers = laporanSetoranColumns.map((c: any) => c.Header).join(',');
    const rows = filteredData.map((row: LaporanSetoran) => 
        laporanSetoranColumns.map((col: any) => (row as any)[col.accessor as string]).join(',')
    );
    ...
    a.setAttribute('download', 'laporan-setoran.csv');
  ...
  return (
    <div className='container-fluid py-10 px-10'>
      <h1 className='mb-4'>Laporan Setoran</h1>
      <KTCard>
        ...
        <div className='card-body'>
          <MetronicTable columns={laporanSetoranColumns} data={filteredData} />
        </div>
      </KTCard>
    </div>
  );
...
export default LaporanSetoranPage; 