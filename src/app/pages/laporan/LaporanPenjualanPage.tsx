...
import { laporanPenjualanColumns, LaporanPenjualan } from './laporanPenjualanColumns';
import { KTCard } from '../../../_metronic/helpers/components/KTCard';
import { KTIcon } from '../../../_metronic/helpers';
import { MenuComponent } from '../../../_metronic/assets/ts/components';

const initialData: LaporanPenjualan[] = [
    { id: 1, tanggal: '2024-06-01', pembeli: 'PT. Kaca Jaya', namaBarang: 'Kaca Bening', jumlah: 100, total: 1200000 },
    { id: 2, tanggal: '2024-06-02', pembeli: 'CV. Warna Abadi', namaBarang: 'Kaca Warna', jumlah: 50, total: 700000 },
];

const LaporanPenjualanPage = () => {
  const [data, setData] = useState<LaporanPenjualan[]>(initialData);
  const [filter, setFilter] = useState({ pembeli: '', tanggal: '' });
  const [filterState, setFilterState] = useState({ pembeli: '', tanggal: '' });
  ...
  const handleResetFilter = () => {
    setFilterState({ pembeli: '', tanggal: '' });
    setFilter({ pembeli: '', tanggal: '' });
  };
  ...
  const handleExport = () => {
    const headers = laporanPenjualanColumns.map((c: any) => c.Header).join(',');
    const rows = filteredData.map((row: LaporanPenjualan) => 
        laporanPenjualanColumns.map((col: any) => (row as any)[col.accessor as string]).join(',')
    );
  ...
  return (
    <div className='container-fluid py-10 px-10'>
      <h1 className='mb-4'>Laporan Penjualan</h1>
      <KTCard>
        ...
        <div className='card-body'>
          <MetronicTable columns={laporanPenjualanColumns} data={filteredData} />
        </div>
      </KTCard>
    </div>
  );
