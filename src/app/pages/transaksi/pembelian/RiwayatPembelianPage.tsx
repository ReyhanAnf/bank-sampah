...
import { riwayatPembelianColumns } from './riwayatPembelianColumns';
...
const initialData: Pembelian[] = [
  ...
  { id: 2, tanggal: '2024-06-02', namaBarang: 'Kaca Warna', jumlah: 50, harga: 600000, total: 600000, pemasok: 'CV. Warna Abadi', status: 'Ditolak' },
];
...
const RiwayatPembelianPage = () => {
  ...
  const handleExport = () => {
    const headers = riwayatPembelianColumns.map(c => c.Header).join(',');
    const rows = filteredData.map(row => 
        riwayatPembelianColumns.map(col => (row as any)[col.accessor as string]).join(',')
    );
    ...
    a.setAttribute('download', 'riwayat-pembelian.csv');
  ...
  return (
    <div className='container-fluid py-10 px-10'>
      <h1 className='mb-4'>Riwayat Pembelian</h1>
      <KTCard>
        ...
        <div className='card-body'>
          <MetronicTable columns={riwayatPembelianColumns} data={filteredData} />
        </div>
      </KTCard>
    </div>
  );
