...
import { verifikasiPenarikanColumns, Penarikan } from './verifikasiPenarikanColumnsNew';
import { KTCard } from '../../../../_metronic/helpers/components/KTCard';
import { KTIcon } from '../../../../_metronic/helpers';
import { MenuComponent } from '../../../../_metronic/assets/ts/components';

const initialData: Penarikan[] = [
  { id: 1, tanggal: '2024-06-01', nasabah: 'Budi', jumlah: 'Rp 100.000', status: 'Menunggu' },
  { id: 2, tanggal: '2024-06-02', nasabah: 'Siti', jumlah: 'Rp 50.000', status: 'Disetujui' },
  { id: 3, tanggal: '2024-06-03', nasabah: 'Ahmad', jumlah: 'Rp 75.000', status: 'Menunggu' },
  { id: 4, tanggal: '2024-06-04', nasabah: 'Rina', jumlah: 'Rp 25.000', status: 'Ditolak' },
];

const VerifikasiPenarikanPage = () => {
  const [data, setData] = useState<Penarikan[]>(initialData);
  ...
  const handleExport = () => {
    const headers = verifikasiPenarikanColumns.map(c => c.Header).join(',');
    const rows = filteredData.map(row => 
        verifikasiPenarikanColumns.map(col => (row as any)[col.accessor as string]).join(',')
    );
    ...
    a.setAttribute('download', 'verifikasi-penarikan.csv');
  ...
  return (
    <div className='container-fluid py-10 px-10'>
      <h1 className='mb-4'>Verifikasi Penarikan</h1>
      <KTCard>
        ...
        <div className='card-body'>
          <MetronicTable columns={verifikasiPenarikanColumns} data={filteredData} />
        </div>
      </KTCard>
    </div>
  );
...
export default VerifikasiPenarikanPage; 